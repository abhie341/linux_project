...
void decryption_mmap_polling(u64 length)
{
...
void encryption_mmap_polling(u64 length)
{
    u8 __iomem *ccbar0 = cc_info.ccbar0;
    u32 len;
    len = (u32)length;
...
void encryption_mmap_interrupt(u64 length)
{
    u8 __iomem *ccbar0 = cc_info.ccbar0;
    u32 len;
    len = (u32)length;
...
void decryption_mmap_interrupt(u64 length)
{
    u8 __iomem *ccbar0 = cc_info.ccbar0;
    u32 len;
    len = (u32)length;
...
void encryption_dma_interrupt(char *data, u64 length)
{
    ...
    iowrite32(0x1|0x4, ccbar0+DMA_CMD_OFF);
    wait_event_interruptible(cc_wq, wq_flag != 0);
    wq_flag = 0;    
...
void decryption_dma_interrupt(char *data, u64 length)
{
    ...
    iowrite32(0x1|0x2|0x4, ccbar0+DMA_CMD_OFF); 
    wait_event_interruptible(cc_wq, wq_flag != 0);
    wq_flag = 0;
    memcpy(data,buff,len);
    dma_free_coherent(dev,len,buff,dma_handle);
...
void encryption_mmio_poll(char *data, u64 length) 
{
	u8 __iomem *ccbar0 = cc_info.ccbar0;
	u32 len;
	len = (u32)length;	
...
void decryption_mmio_poll(char *data, u64 length)
{
	u8 __iomem *ccbar0 = cc_info.ccbar0;
	u32 len;
	len = (u32)length;
...
void encryption_dma_poll(char *data, u64 length)
{
    ...
    while(ioread32(ccbar0+DMA_CMD_OFF) & 1) {;}   
    printk(KERN_INFO "Device is free...START OP\n");
    iowrite32(0x1, ccbar0+DMA_CMD_OFF);   
    while(ioread32(ccbar0+DMA_CMD_OFF) & 1){;}
    memcpy(data,buff,len);
    dma_free_coherent(dev,len,buff,dma_handle);
}
...
void decryption_dma_poll(char *data, u64 length)
{
	...
	while(ioread32(ccbar0+DMA_CMD_OFF) & 1) ;
    	printk(KERN_INFO "Device is free...START OP\n");
    	iowrite32(0x1|0x2, ccbar0+DMA_CMD_OFF);
	while(ioread32(ccbar0+DMA_CMD_OFF) & 1) ;
	memcpy(data,buff,len);
	dma_free_coherent(dev,len,buff,dma_handle);
}
...
{
    	struct cryptocard_config *config; 
			...
			if(isMapped == 0) {
				...
				if (dma == 0) {						
					if (interrupt == 0) {
						...
						if(copy_to_user(ccop->addr, buff, length))
							return EFAULT;
					}
					else {
...
static int cryptocard_chardev_setup(void)
{
	...
	if (IS_ERR(cc_class))
   		goto error_class;
	cc_class->devnode = cryptocard_devnode;   	
...
static int cryptocard_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	ret=request_threaded_irq(irq, cryptocard_irq_handle, NULL, 0, DRIVER_NAME, pdev);	
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("ABHINAV KURUMA");
MODULE_DESCRIPTION("Driver for PCI Device");
MODULE_VERSION("0.1");
